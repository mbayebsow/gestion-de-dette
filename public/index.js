...
function initApp() {
  const app = {
    cleActivation: "appQay5lBp4zkiMR8",
    activationSatus: "",
    ...
    activationKey() {
      ...
      myHeaders.append("Authorization", "Basic patws2H0GoAKpJzOF.9dff3617953ecc78753f0c0979992227ba1c157d03e0306151566b4bbdbf7814");
      ...
      dBSettings.insert(
        ...
        function (err, newDoc) {
          ...
          if (newDoc) {
            ...
            _this.add({ type: "success", text: "Heureux de vous revoir " });
            _this.loadDatas();
      ...
      _this.activationSatus = "";
      _this.add({ type: "success", text: "Go ->" });
      ...
      //       } else {
      //         _this.activationSatus = "";
      //         _this.add({ type: "error", text: "Cette clé nest pas autorisé pour ce logiciel." });
      //         return;
      //       }
      //     } else {
      //       _this.activationSatus = "";
      //       _this.add({ type: "error", text: "Une érreur c'est produit lors de lactivation. Vérifiez votre clé." });
      //       return;
      //     }
      //   })
    ...
    async getDueHistory(id) {
      ...
      const dueHistorySort = dueHistory.sort(function (a, b) {
    ...
    CheckPaiement(id) {
      const obj = this.paiements.filter(function (hero) {
        return hero.DueId == id;
      });
    ...
    CheckSupplements(id) {
      const obj = this.supplements.filter(function (hero) {
        return hero.DueId == id;
      });
      const sumArr = obj.reduce((a, b) => a + b.Montant, 0);
      return sumArr;
    ...
    TotalDue(id) {
      const obj = this.dues.filter(function (Due) {
        return Due.client == id && Due.DueType == "entrant";
      });
      const sumArr = obj.reduce((a, b) => a + b.Montant, 0);
      return sumArr;
    },
